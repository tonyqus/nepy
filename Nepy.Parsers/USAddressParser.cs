...
namespace Nepy.Parsers
{
    ...
    public class USAddressParser : IParser
    {
        ...
        /// <summary>
        /// Initializes the <see cref="USAddressParser"/> class.
        /// </summary>
        static USAddressParser()
        {
        ...
        private static void InitializeRegex()
        {
            ...
            var rangelessSecondaryUnitPattern =
                ...
                string.Join(
                    "|",
                    new string[] { string.Join("|", rangelessSecondaryUnits.Keys.ToArray()) }) +
                @")\b";
        ...
        private static Dictionary<string, string> Normalize(Dictionary<string, string> extracted)
        {
            ...
            if (extracted.ContainsKey("SECONDARYNUMBER") &&
                (!extracted.ContainsKey("SECONDARYUNIT") ||
                 string.IsNullOrEmpty(extracted["SECONDARYUNIT"])))
            {
        ...
        public ParseResultCollection Parse(int startIndex)
        {
            ...
            string input = context.Text.Substring(startIndex);
            if (context.Pattern != ParserPattern.NorthAmerica)
            {
                throw new InvalidOperationException("To use USAddressParser, Parser pattern must be NorthAmerica");
            }
            ...
            if (!string.IsNullOrEmpty(input))
            {
                ...
                if (match.Success)
                {
                    ...
                    var addr = new Address(Normalize(extracted));
                    prc.Add(ParseResult.Create(addr.ToString(), startIndex, POSType.A_M, addr));
                }
            }
            return prc;
        }
